...
class Matrixassistant:
    def load_config(self):
        ...
        return {"roi": None, "grid": None, "lock": None, "matrix_size": None, "speed": "0.3", "scroll_pixel_dist": "200"}

    def load_corrections(self):
        return json.load(open(self.corrections_file, 'r', encoding='utf-8')) if os.path.exists(self.corrections_file) else {}
    ...
    def save_config(self):
        try: self.data.update({"speed": self.speed_var.get(), "scroll_pixel_dist": self.dist_var.get()})
        except: pass
        json.dump(self.data, open(self.config_file, 'w', encoding='utf-8'), ensure_ascii=False, indent=4)
        self.update_config_status()

    def set_matrix_roi(self):
        SelectionCanvas(self.root, "guide_matrix.png", lambda x, y, w, h: [self.data.update({"matrix_size": (w, h)}), self.save_config()])
    ...
    def set_grid(self):
        def p3(rx, ry): gx, gy = self.get_game_rect(); p11 = self.data["grid"]["p11"]; self.data["grid"].update(
            {"rx": p11[0] - gx, "ry": p11[1] - gy, "rdx": self.data["grid"]["p12"][0] - p11[0], "rdy": ry - p11[1]}); self.save_config()
    ...
    def add_weapon_popup(self):
        p = tk.Toplevel(self.root); p.title("添加武器数据"); p.geometry("320x450"); p.attributes("-topmost", True)
        fields = [("武器名称:", "name"), ("星级 (5或6):", "star"), ("毕业词条1:", "c1"), ("毕业词条2:", "c2"), ("毕业词条3:", "c3")]
        ...
        def confirm():
            ...
            self.weapon_list.append(new); messagebox.showinfo("成功", f"武器 {n} 已添加"); p.destroy()
        tk.Button(p, text="确认添加", command=confirm, bg="#2E7D32", fg="white", font=("微软雅黑", 10, "bold"), width=20).pack(pady=25)

    def add_correction_popup(self):
        p = tk.Toplevel(self.root); p.title("错字纠正"); p.geometry("300x180"); p.attributes("-topmost", True)
        w_ent, r_ent = tk.Entry(p, width=15), tk.Entry(p, width=15)
        tk.Label(p, text="错误文字").grid(row=0, column=0, padx=10, pady=10); tk.Label(p, text="正确文字").grid(row=0, column=1, padx=10, pady=10)
        w_ent.grid(row=1, column=0, padx=10, pady=5); r_ent.grid(row=1, column=1, padx=10, pady=5)
        def confirm():
            w, r = w_ent.get().strip(), r_ent.get().strip()
            if w and r: self.corrections[w] = r; json.dump(self.corrections, open(self.corrections_file, 'w', encoding='utf-8'), ensure_ascii=False, indent=4); p.destroy()
    ...
    def capture_window_bg(self, hwnd):
        try:
            ...
            hDC = win32gui.GetWindowDC(hwnd); mDC = win32ui.CreateDCFromHandle(hDC); sDC = mDC.CreateCompatibleDC(); sBM = win32ui.CreateBitmap()
            sBM.CreateCompatibleBitmap(mDC, w, h); sDC.SelectObject(sBM)
            ctypes.windll.user32.PrintWindow(hwnd, sDC.GetSafeHdc(), 2)
            bits = sBM.GetBitmapBits(True); img = np.frombuffer(bits, dtype='uint8'); img.shape = (h, w, 4)
            win32gui.DeleteObject(sBM.GetHandle()); sDC.DeleteDC(); mDC.DeleteDC(); win32gui.ReleaseDC(hwnd, hDC)
    ...
    def is_already_locked_bg(self, window_img, lock_pos):
        try:
            ...
            # 4. 核心逻辑判定：
            # 未锁定：按钮底色是纯白，白色占比极大（通常 > 0.5）
            # 已锁定：按钮底色是深灰，只有中间锁头是白，白色占比小（通常 < 0.4）
            # 我们取 0.45 作为分割线，占比小于 0.45 即判定为已锁定
    ...
    def clean_text(self, raw):
        if not raw: return ""
        txt = self.cc.convert(str(raw)); txt = re.sub(r'[^\u4e00-\u9fff，]', '', txt)
        if self.corrections:
    ...
    def check_all_attributes(self, weapon, ocr_full):
        ...
        for t in ts:
            t_c = t.replace("提升", ""); best_r, b_idx = 0, -1
            for i, p in enumerate(pts):
                if i in m_idx: continue
                r = difflib.SequenceMatcher(None, t_c, p.replace("提升", "")).ratio()
                if r > best_r: best_r, b_idx = r, i
            if best_r >= 0.85: h_hits += 1; m_idx.add(b_idx)
            elif best_r >= 0.6: p_hits += 1; m_idx.add(b_idx)
        return (h_hits == len(ts)) or (h_hits >= len(ts) - 1 and (h_hits + p_hits) >= len(ts))

    def is_gold(self, bgr):
        try:
            h, w = bgr.shape[:2]; strip = bgr[int(h * 0.70):, :]
            hsv = cv2.cvtColor(strip, cv2.COLOR_BGR2HSV); mask = cv2.inRange(hsv, np.array([15, 100, 100]), np.array([35, 255, 255]))
            return (np.sum(mask > 0) / mask.size) > 0.05
        except: return False

    def start_thread(self):
        if not all(self.data.get(k) is not None for k in ["roi", "grid", "lock", "matrix_size"]): messagebox.showwarning("提示", "配置未完成"); return
        self.save_config(); self.corrections = self.load_corrections(); self.log_area.delete('1.0', tk.END); self.lock_list_area.delete('1.0', tk.END)
        self.gui_log("[系统] 扫描启动，按 'B' 键停止", "blue"); self.running = True; self.run_btn.config(state="disabled", text="正在扫描...")
        threading.Thread(target=self.run_task, daemon=True).start()

    def run_task(self):
        try:
            roi, grid, lock, ms = self.data["roi"], self.data["grid"], self.data["lock"], self.data.get("matrix_size", (100, 100))
            hwnd = win32gui.FindWindow(None, 'Endfield') or win32gui.FindWindow(None, '终末地')
            curr_row = 0
            while self.running:
                time.sleep(0.01); spd, dist = float(self.speed_var.get() or 0.3), int(self.dist_var.get() or 200)
                ...
                for c in range(9):
                    ...
                    if self.debug_gold_var.get() or self.is_gold(win_img[max(0, ry - int(ms[1] / 2)):ry + int(ms[1] / 2), max(0, rx - int(ms[0] / 2)):rx + int(ms[0] / 2)]):
                        self.gui_log(f"--- 检查: {curr_row + 1}-{c + 1} ---");
                        wr = self.get_game_rect()
                        pydirectinput.click(int(wr[0] + rx), int(wr[1] + ry)); time.sleep(spd)
                        scr = self.capture_window_bg(hwnd);
                        o_img = scr[int(roi[1]):int(roi[1] + roi[3]), int(roi[0]):int(roi[0] + roi[2])]
                        res, _ = self.ocr(cv2.cvtColor(cv2.resize(cv2.cvtColor(o_img, cv2.COLOR_BGR2GRAY), None, fx=1.5, fy=1.5, interpolation=cv2.INTER_NEAREST), cv2.COLOR_GRAY2BGR))
                        ...
                        if ft:
                            ...
                            if matches:
                                ...
                                if self.is_already_locked_bg(scr, lock): self.gui_log("该基质已锁定，跳过", "red")
                                else:
                                    pydirectinput.click(int(wr[0] + lock[0]), int(wr[1] + lock[1]))
                                    self.gui_log("-> 已执行锁定指令", "blue"); time.sleep(0.4); pydirectinput.moveRel(50, 50)
                                self.add_to_lock_list(matches, f"{curr_row + 1}-{c + 1}")
                        else: self.gui_log("-> 未读到词条")
                    else: self.gui_log(f"非金色基质，停止扫描"); self.running = False; break
                if not self.running: break
                if curr_row >= 4:
                    self.gui_log(f"[翻页] 向上滑动 {dist} 像素...", "black"); wr = self.get_game_rect()
                    sx, sy = int(wr[0] + grid["rx"] + 4 * grid["rdx"]), int(wr[1] + grid["ry"] + 4 * grid["rdy"])
                    pydirectinput.moveTo(sx, sy); pydirectinput.mouseDown(); time.sleep(0.1)
                    for s in range(16): pydirectinput.moveTo(sx, int(sy - (dist * (s / 15)))); time.sleep(0.01)
                    pydirectinput.mouseUp(); time.sleep(1.2)
                curr_row += 1
        except Exception as e: self.gui_log(f"[异常] {e}", "red")
        finally: self.root.after(0, lambda: self.run_btn.config(state="normal", text="▶ 开始自动扫描"))

    def gui_log(self, m, tag="black"):
        self.log_area.insert(tk.END, m + "\n", tag); self.log_area.see(tk.END)

    def add_to_lock_list(self, ms, p):
        for w in ms: self.lock_list_area.insert(tk.END, f"{w.get('武器', '未知')} ", "red_text" if "6" in w.get('星级', '6') else "gold_text")
        self.lock_list_area.insert(tk.END, " " + "，".join([ms[0].get(f'毕业词条{i}', '') for i in range(1, 4) if ms[0].get(f'毕业词条{i}', '')]) + " ", "green_text")
        self.lock_list_area.insert(tk.END, "坐标" + p + "\n", "black_text"); self.lock_list_area.see(tk.END)

    def on_press(self, k):
        if hasattr(k, 'char') and k.char == 'b' and self.running: self.gui_log("[停止] 任务已中止", "red"); self.running = False
    ...
    def get_click(self, p, cb, img_n=None):
        ...
        def onc(e):
            ...
            ov.destroy(); cb(e.x_root, e.y_root)
        ov.bind("<Button-1>", onc); tk.Label(ov, text=p, font=("微软雅黑", 22, "bold"), fg="red", bg="white").pack(expand=True)

    def __init__(self, root):
        self.root = root
        self.root.title("毕业基质自动识别工具beta v1.5 -by洁柔厨")
        self.root.geometry("540x880"); self.root.attributes("-topmost", True)
        ...
        except Exception as e: messagebox.showerror("初始化失败", str(e))
        self.running = False; self.data = self.load_config(); self.weapon_list = self.load_weapon_csv(); self.corrections = self.load_corrections()
        header = tk.Frame(root); header.pack(anchor="nw", padx=10, pady=5, fill="x")
        lf = tk.Frame(header); lf.pack(side="left", anchor="nw")
        self.top_status_var = tk.StringVar(); self.update_config_status()
        tk.Label(lf, textvariable=self.top_status_var, font=("微软雅黑", 9), fg="green").pack(anchor="w")
        tk.Button(lf, text="添加错字纠正", command=self.add_correction_popup, font=("微软雅黑", 8), bg="#F5F5F5", padx=2, pady=0).pack(anchor="w", pady=(2, 0))
        tk.Button(lf, text="添加武器数据", command=self.add_weapon_popup, font=("微软雅黑", 8), bg="#F5F5F5", padx=2, pady=0).pack(anchor="w", pady=(2, 0))
        self.debug_gold_var = tk.BooleanVar(value=False); tk.Checkbutton(lf, text="关闭金色识别", variable=self.debug_gold_var, font=("微软雅黑", 8)).pack(anchor="w", pady=(2, 0))
        rf = tk.Frame(header); rf.pack(side="left", anchor="nw", padx=(10, 0))
        r1 = tk.Frame(rf); r1.pack(anchor="w")
        tk.Label(r1, text=" | 速度:").pack(side="left")
        self.speed_var = tk.StringVar(value=self.data.get("speed", "0.3")); tk.Entry(r1, textvariable=self.speed_var, width=5).pack(side="left", padx=2)
        tk.Label(r1, text=" | 翻页距离:").pack(side="left")
        self.dist_var = tk.StringVar(value=self.data.get("scroll_pixel_dist", "200")); tk.Entry(r1, textvariable=self.dist_var, width=5).pack(side="left", padx=2)
        r2 = tk.Frame(rf); r2.pack(anchor="w", pady=(2, 0))
        ...
        self.run_btn = tk.Button(rf, text="▶ 开始自动扫描", command=self.start_thread, bg="#2E7D32", fg="white", font=("微软雅黑", 12, "bold"), width=15, height=1); self.run_btn.pack(anchor="center", pady=(10, 0))
        mid = tk.Frame(root); mid.pack(pady=5)
        ...
        self.log_area = scrolledtext.ScrolledText(root, height=10, width=60, font=("微软雅黑", 12)); self.log_area.pack(padx=10, pady=5)
        for t, c in [("black", "black"), ("green", "#2E7D32"), ("gold", "#FF9800"), ("red", "red"), ("blue", "blue")]: self.log_area.tag_config(t, foreground=c)
        tk.Label(root, text="已锁定列表:", font=("微软雅黑", 11, "bold")).pack(anchor="w", padx=10)
        self.lock_list_area = scrolledtext.ScrolledText(root, height=8, width=60, font=("微软雅黑", 12), bg="#F9F9F9"); self.lock_list_area.pack(padx=10, pady=5, fill="x")
        for t, c in [("red_text", "red"), ("gold_text", "#FF9800"), ("green_text", "#2E7D32"), ("black_text", "black")]: self.lock_list_area.tag_config(t, foreground=c)
        self.kb = keyboard.Listener(on_press=self.on_press); self.kb.start()
        tk.Label(root, text="群号: 1006580737\n本工具完全免费", font=("微软雅黑", 9, "bold"), fg="#FF5722", justify="right").place(relx=1.0, x=-10, y=10, anchor="ne")
...
if __name__ == "__main__":
    if run_as_admin():
        ...
        sys.excepthook = handle_exception; app = Matrixassistant(root); root.mainloop()